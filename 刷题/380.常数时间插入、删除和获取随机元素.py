...
数组支持随机访问，其按照索引查询的时间复杂度为O(1)O(1),按值查询的时间复杂度为O(N)O(N)， 而插入和删除的时间复杂度为O(N)O(N)。

链表不支持随机访问，其查询的时间复杂度为O(N)O(N),但是对于插入和删除的复杂度为O(1)O(1)（不考虑找到选要处理的节点花费的时间）。

对于哈希表，正常情况下其查询复杂度平均为O(N)O(N)，插入和删除的复杂度为O(1)O(1)。

由于题目要求 getRandom 返回要随机并且要在O(1)O(1)复杂度内，那么如果单纯使用链表或者哈希表肯定是不行的。

而又由于对于插入和删除也需要复杂度为O(1)O(1)，因此单纯使用数组也是不行的，因此考虑多种使用数据结构来实现。

实际上 LeetCode 设计题，几乎没有单纯一个数据结构搞定的，基本都需要多种数据结构结合，这个时候需要你对各种数据结构以及其基本算法的复杂度有着清晰的认知。

对于 getRandom 用数组很简单。对于判断是否已经有了存在的元素，我们使用哈希表也很容易做到。因此我们可以将数组随机访问，以及哈希表O(1)O(1)按检索值的特性结合起来，即同时使用这两种数据结构。
